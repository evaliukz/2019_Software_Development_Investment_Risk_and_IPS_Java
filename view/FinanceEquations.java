...
public class FinanceEquations {
	...
	/*
	 * This class calculates the portfolio return and risk, aka standard deviation,
	 * based on the finance algrithom. For further information on the math model,
	 * please refer to finance equations from
	 * https://www.wallstreetmojo.com/portfolio-standard-deviation/
	 * 
	 * 
	 * The constant numbers used below are based on the historical performance data
	 * during 04/06/2009 - 05/07/2019 of various asset classes Data from
	...
	/*
	 * Method to get portfolio return given the weight of each asset, denoted as
	 * Coefficient parameter below
	 */
	...
	public double getPortfolioReturn(double[] Coefficient) {
		...
		// the constant numbers below are individual asset returns from the database of
		// each asset during 2009-2019
		// Data from https://www.portfoliovisualizer.com/asset-class-correlations
		double[] Individualreturn = { 0.1166, 0.1126, 0.1132, 0.0085, 0.0301, 0.0652, 0.0265, 0.0351, 0.0348, 0.0459,
				0.017, 0.1021, 0.0137 };
		double sum = 0.0;
		for (int i = 0; i < 13; i++) {
	...
	/*
	 * Method to calculate portfolio standard deviation given the weight of each
	 * asset, denoted as Coefficient parameter below
	 */
	// Data from https://www.portfoliovisualizer.com/asset-class-correlations
	...
	public double getPortfolioStandardDeviation(double[] Coefficient) {
		...
		double PortfolioStandardDeviation = Math.sqrt(Math.pow(VTIallocation * VTIstdv, 2)
				+ Math.pow(VOallocation * VOstdv, 2) + Math.pow(VBallocation * VBstdv, 2)
				+ Math.pow(SHYallocation * SHYstdv, 2) + Math.pow(BNDallocation * BNDstdv, 2)
				+ Math.pow(TLTallocation * TLTstdv, 2) + Math.pow(TIPallocation * TIPstdv, 2)
				+ Math.pow(MUBallocation * MUBstdv, 2) + Math.pow(VEUallocation * VEUstdv, 2)
				+ Math.pow(VSSallocation * VSSstdv, 2) + Math.pow(VWOallocation * VWOstdv, 2)
				+ Math.pow(VNQallocation * VNQstdv, 2) + Math.pow(GLDallocation * GLDstdv, 2)
